...
- **Hardening (November 6, 2025 evening):** Capped OpenAI client retries at 3 attempts, added tool-level retry ceilings in `single_agent_tools.py` (skip + telemetry once the limit is hit), trimmed session transcripts to a 20-message rolling window with archival summaries, enforced revision requests to supply an existing `session_id` (API guard + workflow short-circuit), and surfaced a `cannot_revise` banner when cached analysis is unavailable instead of replaying the full pipeline. Tests `backend/tests/analytics/test_revision_routing.py::test_revision_rejected_when_artifacts_missing` cover the regression path.
