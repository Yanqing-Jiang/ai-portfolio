...
\n## 2025-10-14 Remediation Updates\n- Market-share intents now default to annual granularity with quarterly overrides while the SQL template strips quarter filters when not needed.\n- Chart dropdown logic honours direct legend labels (e.g., 'Market Share Percent') and backend sanitisation keeps ECharts series numeric.\n- PlannerExecutor launches stock/web accessories immediately after SQL, cutting analysis wait time by overlapping fan-out with chart generation.\n- Multi-agent orchestration canonicalises tool aliases, deduplicates fan-out payloads, and emits specialist cards so SQL/Web/Stock/Chart render once per run.\n- WorkflowCanvas exposes pan/zoom/reset controls so users can inspect orchestration lanes and reposition nodes as they debug concurrency.\n
