...
- `backend/analytics/controllers/single_agent_controller.py:_agentic_event_stream` and `backend/analytics/flows/planner_executor_flow.py:_stream_with_tool_state` still rely on sequencer-driven telemetry; they must wrap every adapter call (`sql_generation`, `chart_revision`, `stock_refresh`, `web_refresh`, `analysis_revision`) with `agent_tool_call` / `agent_tool_complete` payloads, including argument digests, retries, elapsed_ms, and skip reasons so ledgers prove planner-directed control.
- `backend/core/session_state_snapshot.py:SessionStateSnapshot.revision_context` plus `backend/analytics/flows/planner_executor_flow.py:initialize_context` currently describe revision receipts conceptually but do not persist `arguments_hash`, `output_hash`, timestamps, or guardrail verdicts; these structures need to be stored and hydrated before the planner chooses whether to reuse or refresh a lane.
- `backend/analytics/hooks/stream_hooks.py:_forward_with_hooks` and `backend/analytics/accessory_receipts.py:AccessoryReceipt.store` are not yet emitting normalized `session_started`, `lane_reused_*`, `workflow_cancelled`, or `workflow_redirect` events with `age_seconds` / `fast_path_latency_ms`, leaving Canvas and telemetry blind to accessory freshness.
- `backend/analytics/flows/multi_agent.py:SupervisorOrchestrator.run_revision`, `backend/analytics/orchestrators/supervisor_orchestrator.py:run`, and `backend/analytics/telemetry/schema.py:AgentTurnEvent` still diverge from the single-agent schema; they must stamp `mode="supervisor"`, emit `agent_supervisor_started`, and forward SDK `agent_turn_start/end` plus specialist receipts so UI timelines stay uniform.
- `src/hooks/useAnalyticsMemoryStream.ts` and `src/components/WorkflowCanvas/WorkflowCanvas.tsx` have not adopted the new SSE envelope; they must consume `mode`, `lane_reused`, guardrail notices, and retry badges so deterministic fresh-run pills and agentic revision turns appear without manual log digging.


### 4.7 Path to Done Criteria (Non-Docs/Test)

- **Criterion 1 – Agent Loop Execution:** Route every revision through `SingleAgentController._agentic_event_stream` or `SupervisorOrchestrator.run_revision`, strip lane pre-seeding out of `PlannerSequencer._run_lane`, and enhance `PlannerExecutorFlow._stream_with_tool_state` so the planner itself issues adapter calls, emits `agent_tool_call/complete`, and can raise `workflow_redirect` when receipts flag stale foundations.
- **Criterion 2 – Context Hydration:** Finish `SessionStateSnapshot.revision_context` to persist hashes, guardrail verdicts, and reasoning blurbs per tool, then hydrate that data inside `PlannerExecutorFlow.initialize_context` so helpers like `ctx.revision_context.should_refresh("web")` govern every revision decision.
- **Criterion 3 – Telemetry + UI Parity:** Teach `_forward_with_hooks` and `PlannerSequencer.mark_lane_complete` to attach monotonic `thought_id`s, `lane_reused_*`, and cancellation banners; immediately afterward, have `useAnalyticsMemoryStream` coalesce `agent_tool_call/complete` into single process cards and render lane chips, retry badges, and freshness tooltips in WorkflowCanvas.
- **Criterion 4 – Accessory & Session Guarantees:** Extend `backend/analytics/accessory_receipts.py` to store `age_seconds`, `fast_path_latency_ms`, and guardrail verdicts per lane, emit those via `accessory_receipt.to_telemetry()`, and ensure `_forward_with_hooks` injects `session_started` (or fallback IDs) ahead of `analysis_ready`; the PowerShell smoke workflow in `scripts/agentic_smoke_test.ps1` should assert that every revision stream contains these markers.
- **Criterion 5 – Multi-Agent Storyline:** Centralize telemetry structures in `backend/analytics/telemetry/schema.py` so `SingleAgentController` and `SupervisorOrchestrator` call identical emitters, have `agents_stream_bridge.py` forward SDK `agent_turn_start/end` plus delegation summaries, and stamp `mode` + specialist role metadata within `backend/analytics/sse/stream_adapter.py` so Canvas timelines stay synchronized regardless of delegation.
