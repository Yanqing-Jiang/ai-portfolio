...
- **Agentic defaults.** Flow selection stays centralized, and every non-DIRECT flow now routes through the AgentRuntime without rollout flags so revisions and follow-ups always emit the canonical agent telemetry; DIRECT remains the deterministic rollback path.
...
- `AgentMemory.load_plan_state()` (`backend/analytics/agent_orchestrator/memory.py`) — Rehydrates `PlanState` from `agents_plan_state`, ensuring resumed agent runs pick up whichever DAG node was left pending without relying on legacy cache shims.
...
- `flows/workflow._env_flag()` (`backend/analytics/flows/workflow.py`) — Shared helper for parsing boolean env toggles (e.g., `ANALYTICS_MEMORY_INSTRUMENT`), guaranteeing consistent behavior across controllers.
...
- Single-agent mode always delegates SQL/chart/analysis lanes to the OpenAI Agents SDK; `AgentsStreamBridge.forward()` converts `RunResultStreaming` events into planner-style SSE payloads (`tool_call_delta`, `tool_call_arguments`, `agent_reasoning`) stamped via `apply_mode_metadata`.
...
- `flows/workflow.py` exposes `get_available_flows`, `_resolve_agentic_revision_flag`, `_lane_available`, `_resolve_lane_ttls`, `_build_revision_inputs_plan`, and the async `analytics_memory_workflow` entry point that FastAPI hits to stream SSE events for any registered flow.
...
- **Fallback environment switches.** Agent flows no longer honor `ANALYTICS_ENABLE_AGENTS` or `AGENTIC_*` gates—`SingleAgentController` and `MultiAgentFlow` always boot the OpenAI runtime, while `_env_flag()` now solely backs instrumentation toggles such as `ANALYTICS_MEMORY_INSTRUMENT`.
- **Session/tool cache compatibility layers.** `core.session_state.SessionStateSnapshot.record_agent_run()` and `agent_orchestrator.memory.AgentMemory.record_tool_receipt()` now rely exclusively on the typed `agents_*` fields (`agents_tool_receipts`, `agents_guardrails`, `agents_plan_state`, `agents_recorded_at`), dropping the legacy `tool_cache["agent"]` shim to meet the Phase 5 kill list.
