...
class WebRetrieverAdapter(BaseToolAdapter):
    ...
    description = "Fetch fresh external context using the Responses API web_search_preview tool."
    ...
    async def execute(self, context: ToolExecutionContext) -> ToolAdapterResult:
        ...
        try:
            ...
            metadata["summary"] = "Web search unavailable (Responses API error)."
            metadata["preview_only"] = True
...
class StockTrackerAdapter(BaseToolAdapter):
    ...
    outputs = ("tickers", "ready", "insights", "bars")
    ...
    async def execute(self, context: ToolExecutionContext) -> ToolAdapterResult:
        ...
        if isinstance(company, str):
            tickers = [company.strip().upper()]
        elif isinstance(company, (list, tuple, set)):
            tickers = [str(item).strip().upper() for item in company if str(item).strip()]

        if not tickers:
            query = (context.query or "").strip()
            inferred = {token for token in re.findall(r"[A-Z]{2,5}", query) if token not in self._TICKER_BLACKLIST}
            if inferred:
                tickers = sorted(inferred)[:3]
        ...
        if not getattr(client, "is_configured", False):
            metadata["summary"] = "Polygon client not configured; waiting for API key."
            metadata["preview_only"] = True
            payload["error"] = "polygon_client_unconfigured"
            return ToolAdapterResult(
                name=self.name,
                status="queued",
                payload=payload,
        ...
        try:
            ...
            error_message = str(exc)
            metadata["summary"] = f"Market data fetch failed for {symbol}."
            metadata["error"] = error_message
            payload["error"] = error_message
            return ToolAdapterResult(
                name=self.name,
