...
def _normalize_topic_entry(topic: Any) -> Optional[Dict[str, Any]]:
    if isinstance(topic, Mapping):
        query_value = str(topic.get("query") or topic.get("label") or "").strip()
        if not query_value:
            return None
        label_value = str(topic.get("label") or query_value).strip() or query_value
        entry: Dict[str, Any] = {"query": query_value, "label": label_value}
    ...
    if isinstance(topic, str):
        ...
        if normalized:
            return {"query": normalized, "label": normalized}
    return None


def _derive_revision_topics(
    directive_topics: Optional[Iterable[Any]],
    *,
    fallback_basis: str,
    user_query: str,
    ...
    for topic in directive_topics or []:
        entry = _normalize_topic_entry(topic)
        if not entry:
            continue
        query_key = entry["query"].strip().lower()
    ...
    try:
        from .workflow import _ensure_dual_topics  # Local import to avoid circular dependency
    except Exception:
        return normalized

    working = _ensure_dual_topics(
        [],
        topic_basis=fallback_basis,
        user_query=user_query,
        analysis_text=analysis_text,
    )
    fallback_normalized: List[Dict[str, Any]] = []
    for entry in working:
        normalized_entry = _normalize_topic_entry(entry)
        if not normalized_entry:
            continue
        query_key = normalized_entry["query"].strip().lower()
        if query_key in seen_queries:
            continue
        seen_queries.add(query_key)
        fallback_normalized.append(normalized_entry)
        if len(fallback_normalized) >= 5:
            break
    return fallback_normalized[:5]


def _split_legacy_tokens(inner: str, separators: Tuple[str, ...]) -> List[str]:
    tokens: List[str] = []
    current: List[str] = []
    depth = 0
...
def _split_legacy_pairs(inner: str) -> List[str]:
    return _split_legacy_tokens(inner, (";",))


def _split_legacy_list(inner: str) -> List[str]:
    return _split_legacy_tokens(inner, (",", ";"))


def _parse_legacy_sequence(raw: str) -> Optional[List[str]]:
    text = raw.strip()
    if text.startswith("@(") and text.endswith(")"):
        inner = text[2:-1]
        if not inner.strip():
            return []
        return _split_legacy_list(inner)
    return None
...
def _coerce_structured_value(value: Any) -> Any:
    ...
    if isinstance(value, Mapping):
        return {str(k): _coerce_structured_value(v) for k, v in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [_coerce_structured_value(item) for item in value]
    if isinstance(value, str):
        stripped = value.strip()
        if not stripped:
            return None
        legacy_map = _parse_legacy_mapping(stripped)
        if legacy_map is not None:
            return legacy_map
        legacy_seq = _parse_legacy_sequence(stripped)
        if legacy_seq is not None:
            return [_coerce_structured_value(item) for item in legacy_seq]
        lowered = stripped.lower()
        if lowered == "system.object[]":
            return []
...
def _parse_legacy_mapping(raw: str) -> Optional[Dict[str, Any]]:
    text = raw.strip()
    if not (text.startswith("@{") and text.endswith("}")):
        return None
    inner = text[2:-1]
    result: Dict[str, Any] = {}
    for pair in _split_legacy_pairs(inner):
        ...
        if not key:
            continue
        result[str(key)] = _coerce_structured_value(value.strip())
    return result


def _coerce_to_mapping(value: Any) -> Optional[Dict[str, Any]]:
    sanitized = sanitize_for_json(value) if value is not None else None
    if isinstance(sanitized, Mapping):
        return {str(k): _coerce_structured_value(v) for k, v in sanitized.items()}
    if isinstance(sanitized, str):
        parsed = _parse_legacy_mapping(sanitized)
        ...
        if isinstance(candidate, Mapping):
            return {str(k): _coerce_structured_value(v) for k, v in candidate.items()}
    return None
...
def _ensure_list(value: Any) -> List[Any]:
    ...
    if isinstance(coerced, list):
        return coerced
    if isinstance(coerced, (tuple, set)):
        return list(coerced)
    return [coerced]


def _extract_intent_payload_from_artifact(intent_artifact: Any) -> Optional[Dict[str, Any]]:
    if intent_artifact is None:
        return None
    payload: Dict[str, Any] = {}
    raw_payload = _coerce_structured_value(getattr(intent_artifact, "raw", None))
    if isinstance(raw_payload, Mapping):
        payload.update({str(k): v for k, v in raw_payload.items()})
    slots_payload = _coerce_structured_value(getattr(intent_artifact, "slots", None))
    if isinstance(slots_payload, Mapping):
        payload.setdefault("slots_detected", {str(k): v for k, v in slots_payload.items()})
    elif slots_payload is not None:
        payload.setdefault("slots_detected", slots_payload)
    payload.setdefault("slots_detected", {})
    payload.setdefault("intent_key", getattr(intent_artifact, "intent_key", None))
    confidence = getattr(intent_artifact, "confidence", None)
    payload.setdefault("confidence", confidence if confidence is not None else 0.0)
    assumptions = payload.get("assumptions")
    coerced_assumptions = _coerce_structured_value(assumptions)
    if isinstance(coerced_assumptions, list):
        payload["assumptions"] = coerced_assumptions
    elif coerced_assumptions is None:
        payload["assumptions"] = []
    else:
        payload["assumptions"] = [coerced_assumptions]
    return payload
...
def _normalize_plan_payload(plan_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in ("metrics", "derived_metrics", "group_by"):
        normalized[key] = _ensure_list(normalized.get(key))
    timeframe_value = _coerce_structured_value(normalized.get("timeframe"))
    if isinstance(timeframe_value, Mapping):
        normalized["timeframe"] = timeframe_value
    elif timeframe_value is None:
        normalized.pop("timeframe", None)
    else:
        normalized["timeframe"] = timeframe_value
    filters_value = _coerce_structured_value(normalized.get("filters"))
    if isinstance(filters_value, Mapping):
        normalized["filters"] = filters_value
    ...
    if isinstance(limit_value, str):
        try:
            normalized["limit"] = int(limit_value)
        except ValueError:
            normalized.pop("limit", None)
    return normalized


def _extract_plan_payload_from_artifact(plan_artifact: Any) -> Optional[Dict[str, Any]]:
    if plan_artifact is None:
        return None
    plan_payload = _coerce_to_mapping(getattr(plan_artifact, "plan", None))
    if not plan_payload:
        criteria_payload = _coerce_to_mapping(getattr(plan_artifact, "criteria", None))
        if criteria_payload:
            candidate_plan = criteria_payload.get("plan")
            if isinstance(candidate_plan, Mapping):
                plan_payload = {str(k): v for k, v in candidate_plan.items()}
            else:
                filtered_keys = {
                    "metrics",
                    "derived_metrics",
                ...
                plan_payload = {str(k): v for k, v in criteria_payload.items() if k in filtered_keys}
    if not plan_payload:
        template_payload = _coerce_to_mapping(getattr(plan_artifact, "template", None))
        if template_payload:
            inferred_metrics = _ensure_list(template_payload.get("metrics"))
            if inferred_metrics:
                plan_payload = {
                    "metrics": inferred_metrics,
                    "granularity": template_payload.get("granularity"),
                    "comparison": template_payload.get("comparison"),
                }
    if not plan_payload:
        return None
    return _normalize_plan_payload(plan_payload)
...
def _build_revision_snapshot_payload(ctx: PlannerPhaseContext) -> Optional[Dict[str, Any]]:
    ...
    if intent_model is None:
        intent_artifact = getattr(ctx.artifacts, "intent", None)
        if intent_artifact is not None:
            intent_payload = _extract_intent_payload_from_artifact(intent_artifact)
            if intent_payload:
                try:
                    intent_model = IntentModel.model_validate(intent_payload)
                except Exception:
                    fallback_payload = {
                        "intent_key": intent_payload.get("intent_key"),
                        "confidence": intent_payload.get("confidence", 0.0) or 0.0,
                        "slots_detected": intent_payload.get("slots_detected") or {},
                        "assumptions": _ensure_list(intent_payload.get("assumptions")),
                    }
                    try:
                        intent_model = IntentModel.model_validate(fallback_payload)
                    except Exception:
                        intent_model = None
    ...
    if plan_model is None:
        plan_artifact = getattr(ctx.artifacts, "plan", None)
        plan_payload: Optional[Dict[str, Any]] = None
        if plan_artifact is not None:
            plan_payload = _extract_plan_payload_from_artifact(plan_artifact)
        if plan_payload:
            try:
                plan_model = QueryPlanModel.model_validate(plan_payload)
            except Exception:
                filtered_payload = {
                    key: value
                    for key, value in plan_payload.items()
                    if key in {"metrics", "derived_metrics", "timeframe", "granularity", "comparison", "statistic", "group_by", "filters", "limit"}
                }
                try:
                    plan_model = QueryPlanModel.model_validate(filtered_payload)
                except Exception:
                    plan_model = None
        if plan_model is not None:
            ctx.provisional_plan = plan_model
            if getattr(ctx, "plan", None) is None:
                ctx.plan = plan_model

    signature = getattr(ctx, "intent_signature", None)
    if signature is None:
        signature = build_intent_signature(intent_model, plan_model)
        if signature:
            ctx.intent_signature = signature
    if signature is None:
        logger.info(
            "[REVISION_SNAPSHOT] missing signature session=%s intent=%s plan=%s intent_signature=%s",
            getattr(ctx, "session_id", None),
            bool(intent_model),
            bool(plan_model),
            bool(getattr(ctx, "intent_signature", None)),
        )
        return None
...
class PlannerPipeline:
    ...
    async def _persist_session_state(
        ...
        *,
        ...
        if revision_payload:
            ...
            logger.info(
                "[REVISION_SNAPSHOT] stored session=%s intent_signature=%s plan_keys=%s",
                getattr(ctx, "session_id", None),
                bool(revision_payload.get("intent_signature")) if isinstance(revision_payload, dict) else False,
                sorted(list(revision_payload.get("plan", {}).keys())) if isinstance(revision_payload.get("plan"), dict) else None,
            )
            updated = True
        ...
        if planner_meta.get("follow_up_route") != route_value:
            ...
            updated = True
        receipts_before = []
        existing_receipts = getattr(ctx, "tool_receipts", None)
        if isinstance(existing_receipts, dict):
            receipts_before = sorted(existing_receipts.keys())
        self._ensure_core_tool_receipts(ctx)
        receipts = getattr(ctx, "tool_receipts", None)
        receipts_after = sorted(receipts.keys()) if isinstance(receipts, dict) else []
        if receipts_after != receipts_before:
            logger.info(
                "[TOOL_RECEIPTS] ensured baseline receipts session=%s intent=%s plan=%s before=%s after=%s",
                getattr(ctx, "session_id", None),
                bool(getattr(ctx, "intent", None)),
                bool(getattr(ctx, "plan", None) or getattr(ctx, "provisional_plan", None)),
                receipts_before,
                receipts_after,
            )
        if isinstance(receipts, dict):
            ...
            updated = True
        persisted_receipts = snapshot.tool_cache.get("tool_receipts")
        if isinstance(persisted_receipts, dict):
            logger.info(
                "[TOOL_RECEIPTS] persisted session=%s keys=%s",
                getattr(ctx, "session_id", None),
                sorted(persisted_receipts.keys()),
            )
        if updated:
    ...
    def _record_tool_receipt_from_event(
        ...
        logger.info(
            "[TOOL_RECEIPTS] ensure start session=%s intent=%s plan=%s provisional=%s existing=%s",
            getattr(ctx, "session_id", None),
            bool(getattr(ctx, "intent", None)),
            bool(getattr(ctx, "plan", None)),
            bool(getattr(ctx, "provisional_plan", None)),
            list(receipts.keys()),
        )
        lane = data.get("lane")
        if not lane:
    ...
    def _ensure_core_tool_receipts(self, ctx: PlannerPhaseContext) -> None:
        receipts = getattr(ctx, "tool_receipts", None)
        if receipts is None:
            receipts = {}
            ctx.tool_receipts = receipts
        ...
        if getattr(ctx, "intent", None) is not None and "intent_detection" not in receipts:
            classification_dump: Optional[Dict[str, Any]] = None
            ...
            intent_slots = getattr(ctx.intent, "slots_detected", None)
            fingerprint = {
                "query": getattr(ctx, "query", None),
                "classification": classification_dump,
                "slots_detected": intent_slots,
            }
            receipt = ToolInvocationReceipt(
                ...
                attempts=1,
                input_hash=_hash_payload(fingerprint),
                metadata={"lane": "classification", "refresh_mode": "baseline"},
            )
            receipt.timestamp = baseline_timestamp
            output_payload = {
                "intent_key": getattr(ctx.intent, "intent_key", None),
                "slots_detected": intent_slots,
            }
            receipt.output_hash = _hash_payload(output_payload)
            receipts["intent_detection"] = receipt
            logger.info(
                "[TOOL_RECEIPTS] recorded intent_detection session=%s",
                getattr(ctx, "session_id", None),
            )

        plan_model = getattr(ctx, "plan", None) or getattr(ctx, "provisional_plan", None)
        if getattr(ctx, "intent", None) is not None and plan_model is not None and "plan_generation" not in receipts:
            if hasattr(plan_model, "model_dump"):
                plan_payload = plan_model.model_dump()
            elif hasattr(plan_model, "dict"):
                plan_payload = plan_model.dict()
            else:
                plan_payload = sanitize_for_json(plan_model)
            fingerprint = {
                "query": getattr(ctx, "query", None),
                "intent_key": getattr(ctx.intent, "intent_key", None),
                "plan": plan_payload,
            ...
            receipt = ToolInvocationReceipt(
                ...
                attempts=1,
                input_hash=_hash_payload(fingerprint),
                metadata={"lane": "intent_planning", "refresh_mode": "baseline"},
            )
            receipt.timestamp = baseline_timestamp
            if plan_payload:
                receipt.output_hash = _hash_payload(plan_payload)
            receipts["plan_generation"] = receipt
            logger.info(
                "[TOOL_RECEIPTS] recorded plan_generation session=%s metrics=%s",
                getattr(ctx, "session_id", None),
                len(plan_payload.get("metrics", [])) if isinstance(plan_payload, dict) else None,
            )
...
async def _initialize_context(self, query: str, session_id: Optional[str]) -> PlannerPhaseContext:
    ...
    async def rehydrate_revision_plan(
        self,
        ctx: PlannerPhaseContext,
        *,
        executed: Optional[Set[str]] = None,
    ) -> bool:
        """Ensure intent + plan metadata exist when initializing a revision context.

        Legacy snapshots created before intent persistence can omit the planner
        fields needed by revision refresh helpers. This utility replays the
        lightweight planner tools so downstream code (e.g. run_analysis_refresh)
        can rely on ctx.intent, ctx.plan/provisional_plan, slot statuses, and
        intent signatures being present.
        """
        executed_tools: Set[str] = set(executed or set())
        registry = self.tool_registry
        tools_rerun: List[str] = []
        session_id = getattr(ctx, "session_id", None)

        def _log_state(stage: str) -> None:
            logger.info(
                "[REHYDRATE] %s session=%s has_intent=%s has_plan=%s has_provisional=%s has_resolution=%s receipts=%s",
                stage,
                session_id,
                ctx.intent is not None,
                ctx.plan is not None,
                ctx.provisional_plan is not None,
                ctx.intent_resolution is not None,
                sorted(ctx.tool_receipts.keys()) if getattr(ctx, "tool_receipts", None) else [],
            )

        _log_state("initial")

        async def _consume(tool_name: str) -> None:
            logger.info("[REHYDRATE] invoking %s session=%s", tool_name, session_id)
            async for _ in registry.invoke(tool_name, self, ctx, executed=executed_tools):
                # Discard emitted events; callers only need the ctx mutations.
                pass
            executed_tools.add(tool_name)
            tools_rerun.append(tool_name)
            await self._persist_session_state(ctx, record_artifacts=True)
            _log_state(f"post_{tool_name}")

        # Ensure classification is hydrated before running intent/plan tools.
        if "classification" not in executed_tools and ctx.classification is None:
            classification_artifact = getattr(ctx.artifacts, "classification", None)
            raw_payload = getattr(classification_artifact, "raw", None) if classification_artifact else None
            if isinstance(raw_payload, Mapping):
                try:
                    ctx.classification = OffTopicClassifierSchema.model_validate(raw_payload)  # type: ignore[arg-type]
                except Exception:
                    ctx.classification = None
            if ctx.classification is None:
                await _consume("classification")

        needs_intent = ctx.intent is None
        needs_plan = (ctx.plan or ctx.provisional_plan) is None
        if not needs_intent and not needs_plan:
            # Ensure downstream derivations like intent_signature stay fresh.
            if ctx.intent and (ctx.plan or ctx.provisional_plan):
                ctx.intent_signature = build_intent_signature(ctx.intent, ctx.plan or ctx.provisional_plan)
            logger.info(
                "[REHYDRATE] no replay needed session=%s intent_key=%s plan_metrics=%s",
                session_id,
                getattr(ctx.intent, "intent_key", None),
                len(getattr(ctx.plan, "metrics", []) if ctx.plan else []),
            )
            return True

        if needs_intent:
            await _consume("intent_detection")
        if ctx.intent is None:
            logger.warning(
                "[REHYDRATE] intent missing after replay session=%s tools_rerun=%s",
                session_id,
                tools_rerun,
            )
            return False

        # Copy slot metadata from latest resolution payload if missing.
        if ctx.intent_resolution:
            slots = ctx.intent_resolution.slots or {}
            if slots and not ctx.slot_statuses:
                ctx.slot_statuses = dict(slots)
            followups = list(ctx.intent_resolution.followups or [])
            if followups and not ctx.slot_followups:
                ctx.slot_followups = followups

        if needs_plan:
            await _consume("plan_generation")
        if ctx.plan is None and ctx.provisional_plan is not None:
            ctx.plan = ctx.provisional_plan

        if ctx.intent and (ctx.plan or ctx.provisional_plan):
            ctx.intent_signature = build_intent_signature(ctx.intent, ctx.plan or ctx.provisional_plan)

        # Update tool receipts for the regenerated steps so telemetry reflects a fresh run.
        timestamp = datetime.utcnow().isoformat()

        def _refresh_receipt(tool_name: str, *, fingerprint: Optional[Dict[str, Any]] = None) -> None:
        ...
        if "intent_detection" in tools_rerun:
            classification_dump: Optional[Dict[str, Any]] = None
            if ctx.classification is not None:
                if hasattr(ctx.classification, "model_dump"):
                    classification_dump = ctx.classification.model_dump()
                elif hasattr(ctx.classification, "dict"):
                    classification_dump = ctx.classification.dict()
            fingerprint = {
                "query": ctx.query,
                "classification": classification_dump,
            }
            _refresh_receipt("intent_detection", fingerprint=fingerprint)

        if "plan_generation" in tools_rerun and ctx.intent is not None:
            fingerprint = {
                "query": ctx.query,
                "intent": getattr(ctx.intent, "intent_key", None),
                "slots_detected": getattr(ctx.intent, "slots_detected", None),
            }
            if ctx.plan:
                if hasattr(ctx.plan, "model_dump"):
                    fingerprint["plan"] = ctx.plan.model_dump()
                elif hasattr(ctx.plan, "dict"):
                    fingerprint["plan"] = ctx.plan.dict()
            _refresh_receipt("plan_generation", fingerprint=fingerprint)

        restored = ctx.intent is not None and (ctx.plan or ctx.provisional_plan) is not None
        if not restored:
            logger.warning(
                "[REHYDRATE] plan missing session=%s tools_rerun=%s has_intent=%s plan=%s provisional=%s",
                session_id,
                tools_rerun,
                ctx.intent is not None,
                ctx.plan is not None,
            ...
            logger.info(
                "[REHYDRATE] success session=%s tools_rerun=%s intent_key=%s plan_metrics=%s",
                session_id,
                tools_rerun,
                getattr(ctx.intent, "intent_key", None),
                len(getattr(ctx.plan, "metrics", []) if ctx.plan else []),
