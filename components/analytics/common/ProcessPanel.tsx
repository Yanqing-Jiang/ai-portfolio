...
const formatDurationMs = (value?: number): string | undefined => {
...
export const ProcessPanel: React.FC<ProcessPanelProps> = ({
  ...
  const renderLedgerDetails = (step: ProcessStep) => {
    ...
    const toolCallEntries = Array.isArray(details.tool_calls)
      ? (details.tool_calls as ToolCallTelemetry[])
    ...
    return (
      ...
      <div className="mt-3 space-y-3">
        ...
        {toolCallEntries.length ? (
          <div className="rounded-xl border border-blue-500/40 bg-blue-600/15 p-3 text-blue-100/90">
            <div className="text-[10px] uppercase tracking-wide text-blue-200">Tool Calls</div>
            <div className="mt-2 space-y-2">
              {toolCallEntries.map((entry, idx) => {
                ...
                const durationLabel = formatDurationMs(entry.elapsed_ms);
                const laneLabel = entry.lane ? formatSlotLabel(entry.lane) : undefined;
                return (
                  <div
                    key={`${entry.tool}-${idx}`}
                    className="rounded-lg border border-blue-500/30 bg-blue-900/30 px-3 py-2 text-[11px] leading-relaxed"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <span className="font-semibold text-blue-50">{entry.tool}</span>
                    ...
                    {entry.details?.arguments && typeof entry.details.arguments === 'string' ? (
                      <div className="mt-1 truncate text-[10px] text-blue-200/80">{entry.details.arguments}</div>
                    ) : null}
                    {entry.details?.status && entry.status === 'error' ? (
                      <div className="mt-1 text-[10px] text-rose-200">{entry.details.status}</div>
                    ) : null}
                  </div>
                );
              })}
              ...
              {analysisSourceEntries.map((source) => {
                const formattedColumns =
                  source.columns && source.columns.length
                    ? `${source.columns.slice(0, 4).join(', ')}${source.columns.length > 4 ? ', â€¦' : ''}`
                    : null;
                const formattedSymbols =
                  source.symbols && source.symbols.length
                    ? `${source.symbols.slice(0, 4).join(', ')}${source.symbols.length > 4 ? ', â€¦' : ''}`
                    : null;
