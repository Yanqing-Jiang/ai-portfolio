...
import { resolveChartSpecOption, applyChartOps, sanitizeStructuredText } from '../utils';
...
import {
  ...
  normalizeSpecialistCard,
  normalizeTopicBranchPayload,
  normalizeWebContext,
  parseAnalysisOverview,
  parseAnalysisSources,
  resolveLane,
  resolveReusedFlag,
} from './useDataTransformers';
import { useStreamOrchestrator } from './useStreamOrchestrator';
import { normalizeLegacyEnvelope } from './useLegacyCompatibility';
...
export const useAnalyticsMemoryStream = (
  ...
  const streamOrchestrator = useStreamOrchestrator({ streamHook });
  useEffect(() => {
  ...
  const handleQuery = async (query: string) => {
    ...
    await streamOrchestrator.startStream(endpoint, (data) => {
      const envelope = normalizeLegacyEnvelope(data, revisionContextRef.current);
      revisionContextRef.current = envelope.revisionContext;
      const {
        eventType,
        eventData,
        rawThoughtId,
