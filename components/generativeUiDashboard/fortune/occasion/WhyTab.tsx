...
import { MechanismCard, DynamicCard, ChineseToggle } from '../shared';
...
export const WhyTab: React.FC<{ isReplay?: boolean }> = ({ isReplay = false }) => {
  ...
  return (
    ...
    >
      ...
      {analysis && (
        <DynamicCard
          title={`${analysis.occasionType} Analysis`}
          description={analysis.description}
          elements={analysis.keyElements}
      ...
      {mechanisms.length === 0 ? (
        <div className="text-center py-8 text-xs text-slate-500">
          Date selection mechanisms loading...
        </div>
      ) : (
        <motion.div
